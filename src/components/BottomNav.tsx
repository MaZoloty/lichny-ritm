...
export default function BottomNav({
  ...
  return (
    <nav className="fixed inset-x-0 bottom-0 z-30 bg-gradient-to-t from-bg/95 via-bg/66 to-bg/0 pb-[env(safe-area-inset-bottom)] pt-3">
      <div className="relative mx-auto w-full max-w-md rounded-t-[1.45rem] border-x border-t border-white/80 bg-white/90 px-1.5 py-1 shadow-[0_-8px_30px_-24px_rgba(47,47,53,0.38)] backdrop-blur">
        {moreOpen && (
        ...
        >
          ...
          <li>
            <button
              ...
              className={`flex h-full min-h-[46px] w-full flex-col items-center justify-center gap-0 rounded-[1rem] px-1 py-0.5 text-[10px] font-medium transition ${
                moreActive || moreOpen
...
function NavLink({ item, active }: { item: NavItem; active: boolean }) {
  ...
  return (
    <Link
      href={item.href}
      className={`flex h-full min-h-[46px] flex-col items-center justify-center gap-0 rounded-[1rem] px-1 py-0.5 text-[10px] font-medium transition ${
